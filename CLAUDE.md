...
A client-side image-to-braille-text converter: drop in an image, get back Unicode braille dot-matrix text art (U+2800–U+28FF) that pastes anywhere monospace text works. Plain HTML/CSS/JS, no build step — opening `index.html` directly (`file://`) must keep working for the core converter, so nothing here should require a bundler or ES modules. Two optional enhancements are the exceptions (see Architecture below), and neither is required for the core converter: "Suppress background" vendors a real runtime dependency (`onnxruntime-web` + a small vision model) and needs `http(s)`, degrading gracefully to no-op under `file://`/offline/blocked rather than breaking anything; "Redraw with AI" needs genuine internet access (not just `http(s)`) to call Google's Gemini API directly from the browser using a user-supplied key - see its own Architecture entry for the security constraints that come with handling that key.
...
- **"Redraw with AI"** (in `script.js`, behind the `aiApiKey`/`aiRedrawBtn` field in `index.html`) — sends the loaded image straight from the browser to Google's Gemini API (`gemini-2.5-flash-image`) via a plain `fetch()` call (not Google's `js-genai` SDK, which adds a header that breaks the CORS preflight for this exact use case - see `JOURNEY.md`), using a key the user pastes in themselves. On success, the returned PNG is routed through the existing `loadFile()` rather than a separate image-loading path. **The API key must never be persisted or logged**: read it fresh from the input on each request, never assign it to a variable that outlives that request, never write it to `localStorage`/`sessionStorage`/a cookie, never add it to the settings permalink (`updateUrl`/`restoreSettingsFromUrl`), and never pass it to `console.log`/`console.error` - reloading the page should clear it completely, since a backend-less app has no other safe place to keep a secret.

Keep the dither.js/script.js split intact: any change to the dithering/packing math belongs in `dither.js` (and should get a test in `tests/dither.test.js`); anything touching the UI, events, or rendering orchestration belongs in `script.js`. Keep the on-device-detection code confined to `saliency.js` + the isolated hooks in `script.js` (the checkbox handler, `requestSubjectMask`, `isBackgroundPixel`) - it must never become load-bearing for the core converter. Same isolation rule for "Redraw with AI": it must never become load-bearing for the core converter, and the API-key handling rules above are non-negotiable even for a "helpful" convenience change (e.g. remembering the key across sessions) - if that's ever wanted, it needs an explicit, informed decision, not a quiet addition.

`tests/dither.test.js` covers the pixel-quantization math and the bit-to-braille-dot mapping (validated against the actual Unicode Braille Patterns dot numbering). `tests/ui.test.js` covers the UI layer — drag-and-drop, paste, file loading and error handling, DOM rendering across all three render modes, exports, the settings permalink, "Suppress background" (both its graceful-degradation path and, in one dedicated slower test, the real model), and "Redraw with AI" (button state, the no-persistence/no-logging guarantees, and each response path against a mocked Gemini endpoint - no test hits the real API) — driven with [Playwright](https://playwright.dev/) directly against `node:test` (not the `@playwright/test` runner), so both suites still run under a single `npm test`. That suite blocks `vendor/` requests by default in its shared test setup so the other ~70 tests don't each pay the on-device model's load/inference cost for a feature they aren't testing; only the dedicated test(s) that need the real model use their own unblocked page.
...
## Referencing other repos for research

This session's GitHub access is scoped to `dwooods/asciify` only — `add_repo` refuses a second repo from a different owner in the same session ("cross-tier adds are not supported"). For a *public* repo you just want to read (e.g. comparing an algorithm against another project's implementation), try `add_repo` anyway first: it often replies that anonymous git reads are already available for that repo and hands back a clone command — use that (`GIT_LFS_SKIP_SMUDGE=1 git clone --depth 1 <url> <path>`) rather than treating the cross-owner refusal as a dead end. That path is read-only (no push, no GitHub API/PR tools against it) and clones to a separate directory outside this repo — fine for reading source to compare techniques, not for anything that needs to modify or PR against the other repo.
