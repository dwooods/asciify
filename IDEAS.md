...
- **PWA manifest + service worker** - shipped: `manifest.webmanifest` + `sw.js` let a browser install the app and load its own shell offline after the first visit. Built independently of this backlog entry (landed via a separate session/PR) while this file was mid-edit - a nice confirmation the idea was worth having on the list.
